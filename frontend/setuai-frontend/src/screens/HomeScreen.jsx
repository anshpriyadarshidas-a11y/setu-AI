    ...
    <div className="flex flex-col items-center justify-center flex-1 px-4 py-12">
      <div className="text-center mb-10">
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <h1 className="text-4xl font-bold text-gray-900 tracking-tight">SetuAI</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-3xl">
        {MODES.map((m) => (
          <button
            ...
            className={`bg-white border-2 rounded-2xl p-6 text-left transition-all cursor-pointer shadow-sm hover:shadow-md ${m.color}`}
          >
            <div className="text-3xl mb-3">{m.icon}</div>
            <div className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full mb-2 ${m.badge}`}>
              {m.label}
            </div>
